...
class SpendingItemView extends ConsumerWidget {
  ...
  final AutoDisposeStreamProviderFamily<List<ItemModel>, String> streamProvider;
  const SpendingItemView(
      {super.key, required this.categoryModel, required this.streamProvider});
        ...
        actions: [
          IconButton(
              onPressed: () {
                ...
                showItemAddDialog(context, categoryModel.id!);
              },
              icon: const Icon(Icons.add))
        ],
      ),
      body: ref.watch(streamProvider(categoryModel.id!)).when(
            data: ((data) {
              return Padding(
